...
| R10.13 | Demonstration film | `submission/video/` | **DONE** — two cuts, 3:09 and 1:26, 1920×1080 H.264 with burned-in captions and a sidecar SRT. Most of the running time is the real console, driven with real clicks by `build/capture_walkthrough.py` and framed by a camera aimed at element positions the browser reported; the rest is typeset from the same scan's output. **No narration track** — see `presenter/video.md`. |

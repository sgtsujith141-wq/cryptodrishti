...
async function boot() {
  ...
  $("filter-class").addEventListener("change", () => {
    ...
    renderSwarm(); renderTable(); renderComposition();
  });
...
function initTheme() {
  const sync = () => {
    ...
    dropTokens();
    renderSwarm(); renderMosca(); renderTable(); renderPlan(); renderComposition();
...
function renderAll() {
  renderVerdict(); renderComposition(); renderColophon();
  renderSwarm(); renderMosca(); renderPlan(); renderTable();
}
...
function renderMosca() {
  ...
  const W = 1000, L = 28, R = 168, AXIS = 104;
  ...
  if (exposure > 0) {
    svg.appendChild(sv("rect", { x: px(Z), y: 14, width: Math.max(0, px(X + Y) - px(Z)),
      height: AXIS - 14, fill: bad, opacity: .10 }));
  }
  ...
  for (let y = 0; y <= span + 1e-6; y += step) {
    const xx = px(y);
    if (xx > W - R + 30) break;
    svg.appendChild(sv("line", { x1: xx, y1: AXIS, x2: xx, y2: AXIS + 5,
      stroke: tok("--rule-2"), "stroke-width": 1 }));
    svg.appendChild(sv("text", { x: xx, y: AXIS + 20, fill: ink3, "font-size": 12,
  ...
  const bar = (name, y, x1, x2, colour, label) => {
    ...
    svg.appendChild(sv("rect", { x: x1, y, width: Math.max(3, x2 - x1), height: 19,
      fill: colour }));
    svg.appendChild(sv("text", { x: LX, y: y + 14, fill: tok("--ink-2"),
      "font-size": 12, "font-family": MONO }, label));
  };
  bar("Y", 20, px(0), px(Y), tok("--unknown"), `${Y} yr migration`);
  bar("X", 52, px(Y), px(Y + X), tok("--weakened"), `${X} yr secrecy`);

  svg.appendChild(sv("line", { x1: px(Z), y1: 8, x2: px(Z), y2: AXIS,
    stroke: bad, "stroke-width": 1.5, "stroke-dasharray": "5 4" }));
  const flip = px(Z) > W - R - 92;
  svg.appendChild(sv("text", { x: px(Z) + (flip ? -8 : 8), y: 12, fill: bad,
    "font-size": 12, "font-weight": 640, "font-family": MONO,
    "text-anchor": flip ? "end" : "start" }, `Q-DAY ${state.qday.likely}`));
  if (exposure > 0) {
    svg.appendChild(sv("text", { x: (px(Z) + px(X + Y)) / 2, y: 92, fill: bad,
      "font-size": 12, "font-weight": 640, "font-family": MONO, "text-anchor": "middle" },
  ...
  if (exposure > 0) {
    ...
    out.textContent = `The worst-exposed asset keeps data readable for ${exposure.toFixed(1)} years `
      + `after Q-Day. Probability of exposure ${(p * 100).toFixed(0)}%.`;
  } else {
  ...
  if (!rows.length) {
    ...
    td.textContent = state.findings.length ? "Nothing matches those filters."
      : (state.scanId ? "This scan found no cryptographic assets."
                      : "No scan loaded — choose a target above and press Run scan.");
